...
class App {
  ...
  async listen() {
    ...
    this.app.post("/uploadFile", upload.single("myFile"), (req, res) => {
     ...
     var fileNamedate = Date.now();
    request.post(
      'https://api.riuman.com/api/CheckAuth', {
        json: {
          email: req.body.email,
        }
      },
         ...
         if (a[0] == 1) {
          ...
          if(req.file===undefined){
            ...
            if(numbers!==null && numbers!==undefined){
              await Promise.all(
                numbers.split(',').map((item,k)=>new Promise((resolve, reject) =>  {
                  if (!isNaN(item || item !== '')) {
                    setTimeout(async () => {
                      const contactId = item.includes('@s.whatsapp.net') ? item.trim() : item.trim() + '@s.whatsapp.net'
                      const result = await clite.isOnWhatsApp(contactId)
                      if (result == undefined) {
                        // console.log("Google");
                        // data.push({
                ...
                if(data.length<=a[1]){
                  ...
                  res.json({
                    res:fileNamedate + "WhatsAppVerifier.csv",
                    status:parseInt(a[1]),
                    data:data.length
                  })
                }
                // if(data.length===parseInt(a[1])){
                //   res.json({
                //     res:fileNamedate + "WhatsAppVerifier.csv",
                //   })
                // }
            ...
            try {
              ...
              request.post(
                'https://api.riuman.com/api/SecretApi', {
                  json: {
                    FileName: fileNamedate + "WhatsAppVerifier.csv",
                    email: req.body.email,
                  }
